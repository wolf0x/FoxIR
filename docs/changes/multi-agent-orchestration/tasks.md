...
- [>] T3.5 门禁：G2 已落地（`phase0_g2_subagent_is_temporary_no_main_session`）；G-concurrency（20-run 60 条无丢失）/ G-resume（kill-9 不重 spawn）由 Phase 0 验收覆盖。§20.7 Skill opt-in Review 清单未触发（Phase 0 worker 维持无 Skill）。2026-09-11（③）：门禁指标固化为可复现报告 artifact（`output/phase0_acceptance.json`）
...
- [>] T6.1 §7.5 收尾：LLM 语义层 Auditor（`managed/auditor.rs` 扩输入）—— 输入 = plan + 全部 SubAgentResult + Manager 回答；对矛盾结果读 events；Pass 单写者落盘 / Fail 降级。当前仅确定性 `audit_aggregate` 门控。
- [>] T6.2 §7.5 聚合失败降级方案 A：`update_plan` 重规划再跑（受 `max_rounds`）；当前仅 B（部分结果+分歧）。
- [>] T6.3 模板接入 ManagedRunner Expert 主循环：`workflow::{sequential,parallel,loop_until}` 独立于 runner 的 `parallel_subtasks` 路径；按 §10 / Step 3 由 Manager 内部按需选择。
...
- [>] T6.5 TaskContract ↔ `update_plan` 双向同步（log.md:296）。
- [>] T6.6 Step 3 前端 UI：完整 SPA Expert 视图（Task Queue / 预算抽屉 / 日志抽屉 / 聚合摘要与证据引用跳转 / 模式切换）+ index.html 容器级视觉打磨；T4.1/T4.2 仅做精简 Agent Card + cancel 接线。
- [>] T6.7 §20.7 Skill opt-in Review 清单：未触发/未做。
- [>] T6.8 门禁聚合 + tests/ 独立目录：`tests/step1_gates.rs` / `tests/phase0_e2e.rs` / `tests/mock_llm.rs` 受纯 bin crate 阻塞（需先建 lib target；T1.10/T2.9）。
